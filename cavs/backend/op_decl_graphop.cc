...
class ExtractOpDecl : public OpDecl {
 public:
  ExtractOpDecl(const OpDef& def) : OpDecl(def) {
    CHECK(def.shape_size() == 1) << def.DebugString();
    CHECK(def.input_size() == 0) << def.DebugString();
  }
...
class GatherOpDecl : public ExtractOpDecl {
 public:
  ...
  void MakeGradient(vector<OpDef>* grad) override {
    ...
    //because the output tensor will be reference by(share memory with)
...
class PullOpDecl : public ExtractOpDecl {
 public:
  ...
  void MakeGradient(vector<OpDef>* grad) override {
    ...
    OpDefBuilder("Push")
      .Input(GetGradientName(op_def_.output(0)))
      .Output("Push_Backward" + std::to_string(GetHash(op_def_)))
      .Device(op_def_)
      .Finalize(&push_def);
    grad->push_back(push_def);
  }
};
...
class EmitOpDecl : public OpDecl {
 public:
  EmitOpDecl(const OpDef& def) : OpDecl(def) {
    CHECK(def.input_size() == 1) << def.DebugString();
    CHECK(def.output_size() == 1) << def.DebugString();
  }
